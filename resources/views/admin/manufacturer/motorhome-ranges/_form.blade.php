<form method="POST" action="{{ $url }}" class="admin-form" enctype="multipart/form-data">
  ...
  <div>
    @if ($media = $motorhomeRange->getFirstMedia('tabContentImage'))
      <img src="{{ $media->getUrl('thumb') }}" alt="{{ $media->name }}">
    @endif
    <label for="tab_content_image">
      Tab Content Image
    </label>
    <input name="tab_content_image" type="file" value="{{ old('tab_content_image', '') }}">
  </div>
