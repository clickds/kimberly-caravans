<div class="border-2 border-silver-gray p-5 mb-10">
  <h2 class="text-endeavour mb-5">{{ $vacancy->title }}</h2>
  <div class="text-lg text-dove-gray mb-10">
    {!! $vacancy->short_description !!}
  </div>
  <div class="flex flex-col md:flex-row text-lg text-dove-gray mb-10">
    <div class="w-1/3 flex flex-col">
      @if($vacancy->salary)
        <div class="flex flex-row mb-5">
          <div class="border-2 h-8 w-8 rounded-full border-shiraz text-shiraz text-center mr-5">
            {!! $vacancy->getCurrencySymbol() !!}
          </div>
          <div>
            Salary: {{ $vacancy->salary }}
          </div>
        </div>
      @endif
      <div class="flex flex-row">
        <div class="text-shiraz h-8 w-8 mr-5">
          @include('site.shared.svg-icons.clock')
        </div>
        <div>
          Closing Date: {{ $vacancy->getClosingDate() }}
        </div>
      </div>
    </div>
    <div class="w-2/3">
      <div class="flex flex-row">
        <div class="text-shiraz h-8 w-8 mr-5 mb-5">
          @include('site.shared.svg-icons.book-viewing')
        </div>
        <div>
          {{ implode(', ', $vacancy->getDealerNames()) }}
        </div>
      </div>
    </div>
  </div>
  <div class="text-h4 w-full bg-endeavour text-white text-center p-2">
    <a href="{{ $vacancy->getDetailPageUrl() }}" class="w-full h-full">Click <span class="text-web-orange">here</span> for full job description and to apply online</a>
  </div>
</div>
