...
class MotorhomeRangePresenter extends BasePresenter
{
    public function numberOfModels(): int
    {
        return $this->wrappedObject->motorhomes->count();
    }

    public function lowestPrice(SitePresenter $sitePresenter): string
    {
        $price = $this->wrappedObject->motorhomes->map(function ($motorhome) {
            // The eager load means only the the current site will be loaded
            $recommendedPrice = $motorhome->sites->first()->pivot->recommended_price;
            $price = $motorhome->sites->first()->pivot->price;
